...
class Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.opt.input_label_h5 != 'none':
            ...
            self.input_short_description = np.load("/home/ai/data/yss/data/long_annotations_clean/_label_key_words_v5.npy")
            # load in the sequence data
            seq_size = self.h5_label_file['labels'].shape
            short_size = self.input_short_description.shape
            ...
            self.short_description = self.input_short_description
            # self.seq_length = seq_size[1]
            self.seq_length = opt.max_length
            self.short_length = short_size[1]
        ...
        # self.bbox = json.load(open("/devdata/Dataset_CT/data2077/bbox.json", 'r'))
        self.frame_map = json.load(open("/home/ai/data/yss/data/long_annotations_clean/frame_map.json", 'r'))
        self.att_map = json.load(open("/home/ai/data/yss/data/long_annotations_clean/att_map.json", 'r'))
    ...
    def SystematicSampling(self, dataMat, number):
        ...
        if k > 0:
            while len(sample) != number:
                floor = math.floor(k * i)
                ceil = math.ceil((i + 1) * k)
                ss = list(range(floor, ceil))
                out = list(dataMat[floor:ceil])
    ...
    def get_captions(self, ix, seq_per_img):
        ...
        prob = np.zeros([seq_per_img], dtype='int')
        seq = np.zeros([seq_per_img, self.seq_length], dtype='int')

        if ncap < seq_per_img:
            # we need to subsample (with replacement)
            seq[:ncap, :] = self.label[ix1:ix2 + 1, :self.seq_length]
            prob[:ncap] = 1
        else:
            seq = self.label[ix1: ix1 + seq_per_img, :self.seq_length]
            prob[:seq_per_img] = 1

        short_description = self.short_description[int(self.info['images'][ix]['id'])]

        return seq, short_description, prob
    ...
    def collate_func(self, batch, split):
        ...
        att_batch = []
        label_batch = []
        short_description_batch = []
        frame_surpervise_batch = []
        att_surpervise_batch = []
        bbox_batch = []
        wrapped = False
        ...
        for sample in batch:
            # fetch image
            tmp_tmp_fc, tmp_tmp_att, tmp_seq, tem_short_seq, tmp_frame_map, tmp_att_map,\
            ix, it_pos_now, tmp_wrapped, tmp_probs = sample
            if tmp_wrapped:
                wrapped = True

            tmp_fc = np.zeros([24, tmp_tmp_fc.shape[-1]], dtype='float32')
            tmp_att = np.zeros([24, tmp_tmp_att.shape[-2], tmp_tmp_att.shape[-1]], dtype='float32')
            tmp_frame_surpervise = np.zeros([seq_per_img, 24], dtype='int')
            tmp_att_supervise = np.zeros([24, 14, 14], dtype='int')
            for i in range(24):
                tmp_fc[i] = tmp_tmp_fc[i]
                tmp_att[i] = tmp_tmp_att[i]
                tmp_att_supervise[i] = tmp_att_map[i]

            for i in range(seq_per_img):
                if i < len(tmp_frame_map):
                    tmp_frame_surpervise[i] = tmp_frame_map[i]
                else:
                    break

            fc_batch.append(tmp_fc)
            att_batch.append(tmp_att)
            frame_surpervise_batch.append(tmp_frame_surpervise)
            att_surpervise_batch.append(tmp_att_supervise)


            probs.append(tmp_probs)
            tmp_label = np.zeros([seq_per_img, self.seq_length + 2], dtype='int')
            tmp_short_description = np.zeros([1, self.short_length + 2], dtype='int')
            if hasattr(self, 'h5_label_file'):
                # if there is ground truth
                tmp_label[:, 1: self.seq_length + 1] = tmp_seq
                tmp_short_description[:, 1: self.short_length + 1] = tem_short_seq
            label_batch.append(tmp_label)
            short_description_batch.append(tmp_short_description)
        ...
        fc_batch, att_batch, label_batch, frame_surpervise_batch, att_surpervise_batch, short_description_batch, \
        gts, short_gts, infos = zip(*sorted(zip(fc_batch, att_batch, label_batch, frame_surpervise_batch, att_surpervise_batch,
                            short_description_batch, gts, short_gts, infos), key=lambda x: 0, reverse=True))
        data = {}
        data['probs'] = np.stack(probs)
        # merge att_feats
        max_att_len = max([_.shape[0] for _ in att_batch])
        data['att_feats'] = np.zeros([len(att_batch), max_att_len, att_batch[0].shape[1],  att_batch[0].shape[2]], dtype='float32')
        data['fc_feats'] = np.zeros([len(att_batch), max_att_len, fc_batch[0].shape[1]], dtype='float32')
        for i in range(len(att_batch)):
            data['att_feats'][i] = att_batch[i]
            data['fc_feats'][i] = fc_batch[i]
        ...
        data['frame_surpervise'] = np.stack(frame_surpervise_batch)
        data['att_surpervise'] = np.stack(att_surpervise_batch)
        data['short_description'] = np.vstack(short_description_batch)
        # generate mask
        ...
        nonzeros = np.array(list(map(lambda x: (x != 0).sum() + 2, data['short_description'])))
        mask_batch_short_description = np.zeros([data['short_description'].shape[0], self.short_length + 2], dtype='float32')
        for ix, row in enumerate(mask_batch_short_description):
            row[:nonzeros[ix]] = 1
        data['short_mask'] = mask_batch_short_description
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            try:
                ...
                print(self.info['images'][ix]['id'], 'wrong')
            # Reshape to K x C
            att_feat = att_feat.reshape(-1, 196, att_feat.shape[-1])
        ...
        if hasattr(self, 'h5_label_file'):
            seq, short_seq, probs = self.get_captions(ix, self.seq_per_img)
        else:
            seq = None

        # bbox = self.bbox[int(self.info['images'][ix]['id'])]
        frame_map = self.frame_map[int(self.info['images'][ix]['id'])]
        att_map = self.att_map[int(self.info['images'][ix]['id'])]

        return (fc_feat,
                att_feat, seq, short_seq, frame_map, att_map,
                ix, it_pos_now, wrapped, probs)
