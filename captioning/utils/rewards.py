...
def get_self_critical_reward(greedy_res, data_gts, gen_result, opt):
    ...
    greedy_res, gen_result = greedy_res.view(batch_size, -1), gen_result.view(batch_size, -1)
    ...
    for i in range(gen_result_size):
        ...
        res[i][0] = res[i][0].replace(' 1203', '')
    for i in range(batch_size):
        res[gen_result_size + i] = [array_to_str(greedy_res[i])]
        res[gen_result_size + i][0] = res[gen_result_size + i][0].replace(' 1203', '')

    gts = OrderedDict()
    for i in range(len(data_gts)):
        gts[i] = [array_to_str(data_gts[i][j]) for j in range(len(data_gts[i]))]
        gts[i] = ' '.join(gts[i])
        gts[i] = [gts[i].replace(' 1203', '')]
